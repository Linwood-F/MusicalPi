...
{
    QString notes[] = {"C","D","E","F","G","A","B"};
    QString accidentals[] = {"bbb","bb","b","","#","##","###"};
    int accidentals_offset = +3;  // add this to spelling return to index above
    canPlay = false;
    MidiFile mfi;
    qDebug() << "Reading file";
    mfi.read(midiFile.toStdString());
    qDebug() << "File read, tracks = " << mfi.getTrackCount() << ", TPQ=" << mfi.getTicksPerQuarterNote()
             << ", Time in quarters=" << mfi.getTotalTimeInQuarters()
             << ", Time in seconds=" << mfi.getTotalTimeInSeconds()
             << ", Time in ticks=" << mfi.getTotalTimeInTicks();
    mfi.joinTracks();    // merge gracks to one timeline
    MidiEvent *ptr;
    for (int i=0; i < mfi.getEventCount(0); i++)
    {
        QString midiDataText;
        ptr = &(mfi[0][i]);   // Point to an event
        if(ptr->isAftertouch())       midiDataText = "Aftertouch - undefined";
        else if(ptr->isController())  midiDataText = "Controller - undefined";
        else if(ptr->isEndOfTrack())  midiDataText = "EndOfTrack - undefined";
        else if(ptr->isLinked())      midiDataText = "Linked - undefined";
        else if(ptr->isMetaMessage()) midiDataText = "MetaMessage - undefined";
        else if(ptr->isMeta())        midiDataText = "Meta - undefined";
        else if(ptr->isNoteOn())
        {
            midiDataText = "NoteOn " + QString::number(ptr->getKeyNumber());
            int base7;
            int accidental;
            ptr->getSpelling(base7, accidental);
            midiDataText += "("+ notes[base7 % 7] + accidentals[accidentals_offset + accidental]  + QString::number(base7 / 7 - 1) + ") @ " + QString::number(ptr->getVelocity());
        }
        else if(ptr->isNoteOff())     midiDataText = "NoteOff " + QString::number(ptr->getKeyNumber());
        else if(ptr->isPatchChange()) midiDataText = "PatchChange - undefined";
        else if(ptr->isPitchbend())   midiDataText = "Pitchbend - undefined";
        else if(ptr->isPressure())    midiDataText = "Pressure - undefined";
        else if(ptr->isTempo())       midiDataText = "Tempo - undefined";
        else if(ptr->isTimbre())      midiDataText = "Timbre - undefined";
        else midiDataText = "Type not defined in function";
        ...
        qDebug() << "Event " << i << " at ticks " << ptr->tick << ", on track " << ptr->track << ", seconds = " << mfi.getTimeInSeconds(0,i) << ", " << midiDataText << ", " << midiDataNumbers;
